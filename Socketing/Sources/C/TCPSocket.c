...
void c_tcp_socket_set_block(int socket, int on) {
    int flags;
...
int c_tcp_socket_connect(const char *host, int port, int timeout) {
    ...
    c_tcp_socket_set_block(sockfd,0);
    ...
    if (retval < 0) {
        ...
        c_tcp_socket_set_block(sockfd, 1);
        int set = 1;
...
int c_tcp_socket_close(int socketfd){
    return close(socketfd);
}

int c_tcp_socket_pull(int socketfd, char *data, int len, int timeout_sec) {
    if (timeout_sec > 0) {
        fd_set fdset;
...
int c_tcp_socket_send(int socketfd, const char *data, int len){
    int byteswrite = 0;
...
//return socket fd
int c_tcp_socket_listen(const char *address, int port) {
    //create socket
...
int c_tcp_socket_accept(int onsocketfd, char *remoteip, int *remoteport, int timeouts) {
    socklen_t clilen;
...
//return socket port
int c_tcp_socket_port(int socketfd) {
